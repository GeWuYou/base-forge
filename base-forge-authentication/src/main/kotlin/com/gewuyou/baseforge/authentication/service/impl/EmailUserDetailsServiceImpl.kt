 ...
 *电子邮件用户详细信息服务实现
 *
 * @since 2025-02-17 23:38:04
 * @author gewuyou
 */
@Service("emailUserDetailsService")
class EmailUserDetailsServiceImpl(private val authorizationOpenfeignClient: BaseForgeAuthorizationOpenfeignClient) : UserDetailsService {
    /**
     * 根据用户身份标识获取用户
    ...
    override fun loadUserByPrincipal(principal: Any): UserDetails {
        // 先获取用户认证信息
        val authUser =  authorizationOpenfeignClient.findByEmail(principal as String)?: throw IllegalArgumentException("用户不存在")
