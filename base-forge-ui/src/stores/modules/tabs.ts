...
import type { TabsMenuProps, TabsState } from '@/models/interface/props'
...
export const useTabsStore = defineStore({
  ...
  actions: {
    ...
    async addTab(tabItem: TabsMenuProps) {
      ...
      // keepalive
      if (
        !keepAliveStore.keepAliveNames.includes(tabItem.path) &&
        tabItem.isKeepAlive
      ) {
        await keepAliveStore.addKeepAlive(tabItem.path)
      }
    },
    // 移除tab
    async removeTab(tabPath: string, isCurrent: boolean = true) {
      ...
      if (tabItem?.isKeepAlive) {
        await keepAliveStore.removeKeepAlive(tabItem.path)
      }
    ...
    async closeTabsOnSide(tabPath: string, type: 'left' | 'right') {
      ...
      await keepAliveStore.setKeepAliveNames(
        keepAliveList.map(item => item.path),
      )
    },
    // 关闭多个tab
  ...
  persist: piniaPersistConfig('tab'),
})
